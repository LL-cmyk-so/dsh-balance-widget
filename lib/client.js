window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		/**
		* Fetch the active session's costs (last prompt + session total).
		* @returns { ok, lastPrompt?, sessionTotal?, error? }
		*/
		async function fetchActiveCost() {
			try {
				const response = await fetch(ACTIVE_COST_ROUTE, { headers: { accept: "application/json" } });
				const payload = await response.json().catch(() => ({}));
				if (!response.ok) return { ok: false, error: payload.error ?? `active-cost responded ${response.status}` };
				return { ok: true, lastPrompt: payload.lastPrompt?.cost ?? 0, sessionTotal: payload.sessionTotal?.cost ?? 0 };
			} catch (error) {
		...
		*/
		function SidebarBalanceCard({ wide, t }) {
			...
			const [sessionTotal, setSessionTotal] = (0, react.useState)(null);
			...
			const fetchTodayCosts = async () => {
				const response = await fetch(TODAY_COST_ROUTE, { headers: { accept: "application/json" } });
				const payload = await response.json().catch(() => ({}));
			...
			const refresh = async (showLoading) => {
				...
				if (aResult.ok) {
					...
					setSessionTotal(aResult.sessionTotal);
				} else {
							...
							children: [(0, react_jsx_runtime.jsx)("div", {
								...
								children: (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, {
									children: [(0, react_jsx_runtime.jsx)("span", {
										className: styles.label,
										children: (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, {
											children: [t("sessionTotal"), (0, react_jsx_runtime.jsx)(ExplainIcon, {
												label: t("explain.sessionTotal"),
												t
											})]
										})
									}), (0, react_jsx_runtime.jsx)("span", {
										className: styles.val,
										children: loading ? "…" : sessionTotal !== null ? formatCny(sessionTotal) : "—"
									})]
								})
							}), (0, react_jsx_runtime.jsx)("div", {
								className: styles.row,
		...
		const zh = {
			...
			"explain.sessionTotal": "最近活跃会话从开始到现在的累计费用估算。",
		...
		const en = {
			...
			"explain.sessionTotal": "Estimated total cost of the most recent session since it started.",
