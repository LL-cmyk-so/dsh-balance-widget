	...
	factory: (require) => {
		...
		*/
		function BalanceWidget({ useProjection, sessionId, t }) {
			...
			const toggle = async () => {
				...
				if (next) {
					...
					else if (!lastResult.ok && lastResult.error) setError((error ?? "") + (error ? " " : "") + `last: ${lastResult.error}`);
					if (todayResult.ok) setTodayCost(todayResult.cost);
					else if (!todayResult.ok && todayResult.error) setError((error ?? "") + (error ? " " : "") + `today: ${todayResult.error}`);
					setLoading(false);
				}
			};
		...
		const en = {
			...
			"lastPromptCost": "Last prompt (est.)",
			"todayCost": "Today total (est.)",
			"tokens": "Token usage",
