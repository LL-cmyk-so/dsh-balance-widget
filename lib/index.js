...
async function resolveSessionFile(sessionId) {
	...
	try {
		...
		for (const dir of dirs) {
			...
			const candidate = `${root}/${dir.name}/session-${sessionId}/session.jsonl.zstd`;
...
/**
 * Decompress a .zstd session file to text via the zstd CLI (present on
 * macOS/Linux with zstd installed). Returns undefined on failure.
 */
...
async function collectAllSessionSamples() {
	...
	for (const file of files) {
		const text = await decompressSession(file);
		if (text === void 0) continue;
		perSession.push(parseSessionUsages(text));
	}
	return perSession;
}
	...
	return [
		...
		},
		{
			...
			handler: async (req, res) => {
				...
				const text = await decompressSession(file);
				if (text === void 0) {
					writeJson(res, 502, { error: "failed to read session file", modelId: config.modelId });
					return;
				}
				const samples = parseSessionUsages(text);
				const result = lastTurnCost(samples);
				writeJson(res, 200, { ...result, modelId: config.modelId });
			}
		},
		{
			...
			handler: async (req, res) => {
				...
				const result = todayCost(perSession);
				writeJson(res, 200, { ...result, modelId: config.modelId });
			}
		}
	];
